...
from .marker import Marker, mint, redact
...
class Injector:
    ...
    #: Called after the local effect may have fired but before it is confirmed.
    #: The journal writes its intent record here, so a crash between the effect
    #: and its confirmation is recoverable as ambiguous rather than invisible.
